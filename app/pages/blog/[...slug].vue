  ...
  <div class="w-full h-full md:p-20 p-10">
    <template v-if="post">
      <div class="flex flex-row items-center my-10 md:my-auto">
        ...
        <BreadCrumb class="md:visible invisible md:size-auto size-0">{{ post.title }}</BreadCrumb>
        <p class="grow font-mono text-sm line-clamp-1 font-extralight text-gray-500 text-right">
          Published <span class="text-black">{{ format(new Date(post.publishedOn), 'MMM do, yyyy') }}</span>
        </p>
      </div>
      <BlogTitle>{{ post.title }}</BlogTitle>
      <BlogDescription>{{ post.description }}</BlogDescription>
      <ContentRenderer class="my-10 border-t-slate-300 border-t py-10" :value="post" />
    </template>
    <div v-else class="h-full flex flex-col justify-center items-center">
      ...
      <button class="transition-all ease-in-out duration-[250ms] delay-100 flex flex-row justify-center items-center bg-gradient-to-r from-indigo-500 to-pink-500 text-white hover:saturate-150 px-2.5 py-2.5 rounded-lg m-2 scale-125 hover:scale-150" @click="router.replace('/blog')">
